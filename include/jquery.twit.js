...
(function(jQuery){
  ...
  var _callback = function (o, e, opts) {
    ...
    if (opts.icon || opts.username) {
      ...
      if (opts.username)
        h += '<label>ANADE A </label>';
		h += '<label>NUEVOS AMIGOS </label>';
      h += '</h3>';
	  h+='<form id="contactFormId"  method="POST" action="addUser.php"><label for="Nombre2">Nombre</label><input type="text" name="Nombre" id="Nombre" size="50"/><label for="IP2">IP</label><input type="text" name="IP" id="IP" size="50"/><p><input type="submit" name="boton" id="boton" value="  ANADIR NUEVO AMIGO  " /></p> </form>';
    }
